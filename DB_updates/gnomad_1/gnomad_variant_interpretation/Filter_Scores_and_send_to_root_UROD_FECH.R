...
final_data <- read.csv("../../Scores_FECH_UROD.csv") %>%
  filter(Func.refGeneWithVer == "exonic") %>%
  select(-Func.refGeneWithVer, -SJHConsScore, -in_vitro_HMBS_activity, 
         -CONDEL, -SJHConsScore, -HGVS_protein_dual, -DiscovEHR
  )
...
thresholds <- c(
  ...
  AlphaMissense = 0.5,  # Threshold for AlphaMissense
  #DiscovEHR = 1,  # DiscovEHR is treated as 1 if not NA, 0 if NA
  BayesDel = 0.07  # Threshold for BayesDel_addAF
)
...
final_data <- final_data %>%
  mutate(
    ...
    AlphaMissense_pass = ifelse(AlphaMissense > thresholds["AlphaMissense"], 1, 0),  # AlphaMissense
    #DiscovEHR_pass = ifelse(!is.na(DiscovEHR) & DiscovEHR == thresholds["DiscovEHR"], 1, 0),  # DiscovEHR
    BayesDel_pass = ifelse(BayesDel
 > thresholds["BayesDel"], 1, 0)  # BayesDel
  )

# Calculate the number of valid predictors (non-NA values) for each row
final_data <- final_data %>%
  mutate(
    Valid_Predictors_Count = rowSums(!is.na(select(., PP2, SIFT, Consurf, MA, PROVEAN, FATHMM, REVEL, CADD, AlphaMissense, BayesDel)))
  )
...
#A lot of variants don't have annovar predictor data scores. 
...
write.csv(final_data_cns, "../../Scores_check_urod_FECH.csv", row.names = FALSE)
